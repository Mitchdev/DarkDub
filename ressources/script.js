	...
	var fcs = {
		...
		"autoVote": function() {
			if (fcs.settings.autoVote) {
				if (!fcs.item.dubUp.hasClass("voted") || !fcs.items.dubDown.hasClass("voted")) fcs.item.dubUp.click();
			}
		},
		"theme": function(){},
		"smallChat": function(){},
		"kill": function() {
			fcs.log("Aurevoir !", "log");
...
function init(kill) {
	...
	if (!localStorage.getItem("fcs-settings")) {
		localStorage.setItem("fcs-settings", fcs.settings);
		fcs.user.settings = fcs.settings;
	} else {
		fcs.user.settings = localStorage.getItem("fcs-settings");
		var i=0,j=0;
	...
	$("#fcs-menu").on("click", "li", function(){
		...
		switch($(this).id) {
			...
			default: console.log("Hey ! Yeah you WiBla, you f*ed up !");
		}
	});
	...
	// loading complete
	fcs.log("FCS $version loaded !", "log");
}
